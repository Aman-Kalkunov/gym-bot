...
export const handleHyrox = async (ctx: Context, messageType: MessageType) => {
  const now = new Date();
  ...
  const daysMap = trainings
    ?.filter(t => t.type === TrainingType.HYROX)
    .reduce(
      (acc, training) => {
  ...
  const buttons = Object.entries(daysMap).map(([day, slots]) => [
    Markup.button.callback(
      ...
      `${HYROXTypes.HYROX_DAY}_${slots[0].dayOfWeek}`,
    ),
  ]);

  buttons.push(
    [Markup.button.callback(AdminButtonsText.ADMIN_BACK, HYROXTypes.HYROX_DAY_BACK)],
    [Markup.button.callback(HYROXTypesText.CLOSE, HYROXTypes.CLOSE)],
  );
...
export const handleHyroxDay = async (ctx: Context, dayOfWeek: number, messageType: MessageType) => {
  const trainings: ITraining[] | null = await prisma.training.findMany({
    where: { dayOfWeek },
    orderBy: { time: 'asc' },
  });
    ...
    .map(slot => {
      ...
      return [
        Markup.button.callback(
          ...
          free > 0 ? `${HYROXTypes.HYROX_TIME}_${slot.id}` : 'NO_SLOTS',
        ),
      ];
    });

  buttons.push(
    [Markup.button.callback(AdminButtonsText.ADMIN_BACK, HYROXTypes.HYROX_TIME_BACK)],
    [Markup.button.callback(HYROXTypesText.CLOSE, HYROXTypes.CLOSE)],
  );
...
export const handleHyroxTime = async (ctx: Context, trainingId: number, adminId: string) => {
  const user = ctx.from;
  if (!user) {
    return;
  }
  ...
  if (free <= 0) {
    ...
    await handleHyroxDay(ctx, training.dayOfWeek, 'reply');
    return;
  }
  ...
  await safeEditOrReply(
    ctx,
    `Вы записаны на тренировку Hyrox: ${training.time} (${getFormatDate(training.date)})`,
  );

  const msg = `${userName} записался(-ась) на тренировку Hyrox: ${training.time} (${getFormatDate(
    training.date,
  )})`;
